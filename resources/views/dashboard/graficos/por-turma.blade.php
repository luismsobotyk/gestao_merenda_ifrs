...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
        <div>
            <h1 class="h2 mb-0">Retiradas por Turma</h1>
            <small class="text-muted">Volume histórico de adesão à merenda escolar separado por curso.</small>
        </div>
    </div>

    {{-- BARRA DE FILTRO COM SELEÇÃO DE GRÁFICO --}}
    <form method="GET" action="{{ route('graficos.por_turma') }}" class="card shadow-sm border-0 mb-4 bg-light">
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-body p-4">
                    <div id="graficoTurmas" style="width: 100%; height: 550px;"></div>
                </div>
            </div>
        </div>
    </div>
...
@section('custom_js')
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            var chartDom = document.getElementById('graficoTurmas');
            var myChart = echarts.init(chartDom);

            var labelsTurmas = {!! json_encode($labels) !!};
            ...
            if (tipoSelecionado === 'pie') {
                var pieData = labelsTurmas.map(function(label, index) {
                    return { name: label, value: dadosValores[index] };
                });

                option = {
                    title: [
                        { text: 'Distribuição por Turmas', left: 'center', top: 0, textStyle: { color: '#495057', fontWeight: 'bold' } },
                        { text: '', left: 'center', top: '45%', textStyle: { fontSize: 18, fontWeight: 'bold', color: '#333', width: 250, overflow: 'truncate' } }
                    ],
                    tooltip: {
                        trigger: 'item',
                ...
                var seriesDinamicas = labelsTurmas.map(function(nome, index) {
                    return {
                        name: nome,
                        type: 'bar',
                ...
                option = {
                    title: { text: 'Distribuição por Turmas', left: 'center', top: 0, textStyle: { color: '#495057', fontWeight: 'bold' } },
                    tooltip: {
                        trigger: 'item',
                        formatter: function(params) {
